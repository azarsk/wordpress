    ...
    if($_POST['oscimp_hidden'] == 'Y') {
		...
		$table_name = $wpdb->prefix .'myplugin';
		 ...
		 $data=array( 
			...
			'storeUrl' => $store_url, 
			'productImageFloder' => $prod_img_folder, 
			);
		  //print_r($data);exit;	
...
<div class="wrap">
    <?php    echo "<h2>" . __( 'My Plugin ', 'oscimp_trdom' ) . "</h2>"; ?>
...
<div class="wrap">
 <?php    echo "<h2>" . __( 'My Plugin ', 'oscimp_trdom' ) . "</h2>"; ?>
 <form name="myplgun_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
   <input type="hidden" name="myplugin_hidden" value="Y">
 <?php    echo "<h4>" . __( 'OSCommerce Database Settings', 'oscimp_trdom' ) . "</h4>"; ?>
   <?php    echo "<h4>" . __( 'OSCommerce Store Settings', 'oscimp_trdom' ) . "</h4>"; ?>
      <p><?php _e("Store URL: " ); ?><input type="text" name="oscimp_store_url" value="" size="20"><?php _e(" ex: http://www.yourstore.com/" ); ?></p>
      <p><?php _e("Product image folder: " ); ?><input type="text" name="oscimp_prod_img_folder" value="" size="20"><?php _e(" ex: http://www.yourstore.com/images/" ); ?></p>
      <p class="submit">
        <input type="submit" name="Submit" value="<?php _e('save', 'oscimp_trdom' ) ?>" />
      </p>
